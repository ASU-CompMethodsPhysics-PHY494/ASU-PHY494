...
Save it to file 'TODO' with the command `^O TODO` and exit with
`^X`. Check that the file is there with `ls`.
...
6. Make a copy of your `notes` directory in your work directory:
...
7. Remove `data/notes/hints.txt` with `rm`.
8. Remove `data/notes` with `rmdir`. (Hint: Read the error message!)
9. Move `work/notes/hints.txt` into the `work` directory.
10. Remove the useless `work/notes` directory with `rm -r` (careful !)
...
browser or if you have the `curl` program installed, try `curl
{{site.url}}{{site.baseurl}}/{{site.data}}/planets.dat -O`.)
...
{% highlight bash %}
cat planets.txt planets.txt > planets_2.txt
{% endhighlight %}
...
`wc -l` gives just the number of lines.

Sort by name and look at the first five using a **pipes** and
**filters**:

{% highlight bash %}
sort planets.txt | head -5
{% endhighlight %}
...
{% highlight bash %}
sort -k2,2 -n planets.txt | head -5 > biggest_planets
{% endhighlight %}

Count the number of planets with unknown diameter:

{% highlight bash %}
grep "unknown" planets.txt | wc -l
{% endhighlight %}

Get the first letter of each planet name and sort alphabetically:

{% highlight bash %}
cut -b 1,1 planets.txt | sort
{% endhighlight %}

### Activity

1. Count the number of planets in `planets_2.txt`.
2. Find planets where the rebel might have a base (hint: you know it's
   ...
   more closely?
3. What is the most frequent and the least frequent first letter
